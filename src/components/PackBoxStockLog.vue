...
<template>
  <div class="container">
    ...
    <div class="query-condition">
      <el-form :inline="true" :model="queryCondition">
        ...
        <el-row>
          ...
          <el-col :span="4">
            <el-form-item>
              <el-button type="primary" @click="findPackBoxStockLogPage">查询</el-button>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <div class="table-data">
      <el-table :data="packBoxStockLogData" border style="width: 80%">
        ...
        <el-table-column prop="packBoxNumber" label="拼盒编号" label-width="120"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" label-width="100">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="showPackBoxDetail(scope.packBoxNumber)">查看拼盒明细</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div class="pack-box-detail-dialog">
      <el-dialog
        ...
        width="70%">
        <el-table :data="packBoxDetailData" border style="width: 80%">
          <el-table-column prop="stockId" label="原库存id" label-width="150"></el-table-column>
          <el-table-column prop="proMaterialNumber" label="料号" label-width="120"></el-table-column>
          <el-table-column prop="proModel" label="型号" label-width="120"></el-table-column>
          <el-table-column prop="lotNumber" label="批号" label-width="120"></el-table-column>
...
<script>
import packBoxStockLogService from '../services/packBoxStockLogService'
export default {
  ...
  methods: {
    ...
    showPackBoxDetail (packBoxNumber) {
      if (this.packBoxNumber !== packBoxNumber) {
        this.packBoxNumber = packBoxNumber
        this.findByPackBoxNumber()
      } else {
        this.dialog.showDialog = true
      }
    },
    findPackBoxStockLogPage () {
      packBoxStockLogService.findPackBoxStockLogPage(this.queryCondition).then(response => {
        console.log(response)
        let responseData = response.responseData
        if (responseData.success) {
          let pageInfo = responseData.findPackBoxStockLogPageInfo
          this.queryCondition.total = pageInfo.total
          this.packBoxStockLogData = pageInfo.records
        } else {
    ...
    findByPackBoxNumber () {
      packBoxStockLogService.findByPackBoxNumber(this.packBoxNumber).then(response => {
        console.log(response)
      })
    }
  }
...
<style scoped>
@import '../assets/style/packBoxStockLog.css';
